...
def criar_grafico_evolucao_precos(
    df: pd.DataFrame, col_data: str = "data_pedido", col_preco: str = "valor_total"
) -> None:
    if df is None or df.empty or col_data not in df.columns or col_preco not in df.columns:
        ux.info("Sem dados suficientes para evolução de preços.")
        return
    d = df.copy()
...
def criar_timeline_compras(df: pd.DataFrame, col_data: str = "data_pedido") -> None:
    if df is None or df.empty or col_data not in df.columns:
        ux.info("Sem dados para timeline.")
        return
    d = df.copy()
    ...
    add_bar_labels(fig, kind="count")
    style_plotly(fig, kind="bar", height=360)
    st.plotly_chart(fig, use_container_width=True)


def criar_ranking_fornecedores_visual(
    df: pd.DataFrame, col_fornecedor: str = "fornecedor_nome", col_preco: str = "valor_total"
) -> None:
    if df is None or df.empty or col_fornecedor not in df.columns:
        ux.info("Sem dados para ranking.")
        return
    d = df.copy()
    if col_preco in d.columns:
        ...
        style_plotly(fig, kind="bar", height=420)
        st.plotly_chart(fig, use_container_width=True)
    else:
...
def criar_mini_mapa_fornecedores(df: pd.DataFrame) -> None:
    ...
    ux.info("Mapa detalhado está na aba 'Mapa Geográfico'.")
