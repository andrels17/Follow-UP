...
-- Auditoria funcional (ações do usuário)
...
-- Logs técnicos do app (eventos, erros)
...
-- Upgrade seguro: campos adicionais para multi-tenant / drilldown (best-effort)
ALTER TABLE app_logs ADD COLUMN IF NOT EXISTS tenant_id UUID;
ALTER TABLE app_logs ADD COLUMN IF NOT EXISTS user_id UUID;
ALTER TABLE app_logs ADD COLUMN IF NOT EXISTS page TEXT;

CREATE INDEX IF NOT EXISTS idx_app_logs_timestamp ON app_logs(timestamp);
CREATE INDEX IF NOT EXISTS idx_app_logs_level ON app_logs(level);
CREATE INDEX IF NOT EXISTS idx_app_logs_event ON app_logs(event);

-- Snapshots diários por tenant (gerados sob demanda – Streamlit Cloud safe)
...
CREATE INDEX IF NOT EXISTS idx_saas_snapshots_date ON saas_snapshots(snapshot_date);
CREATE INDEX IF NOT EXISTS idx_saas_snapshots_tenant ON saas_snapshots(tenant_id);


-- ============================
-- Mapeamento Departamento -> Almoxarifado
-- ============================
create table if not exists public.depto_almox_map (
  id bigserial primary key,
  tenant_id uuid not null,
  departamento text not null,
  almoxarifado text not null,
  created_at timestamptz not null default now(),
  unique (tenant_id, departamento)
);
